...
func TestValidateRedirectURI(t *testing.T) {
	cases := []struct {
		...
		{"https ok", "https://sso.gerege.mn/sso/callback", false, true},
		{"http loopback ok", "http://127.0.0.1:8080/cb", false, true},
		{"http non-loopback rejected", "http://sso.gerege.mn/cb", false, false},
		...
		{"fragment rejected", "https://sso.gerege.mn/cb#x", false, false},
	}
	for _, c := range cases {
...
func TestValidateAdminClient(t *testing.T) {
	...
	if err := validateAdminClient(adminClientBody{Name: "app", RedirectURIs: []string{"https://sso.gerege.mn/cb"}}, true); err != nil {
