...
.PHONY: help build tidy test test-integration test-cover swag mock lint fmt \
...
mock: ## Generate mock for an interface (usage: make mock interface=Name dir=path filename=mock_name.go)
	...
	cd ./internal/mocks && mv $(interface).go $(filename).go
