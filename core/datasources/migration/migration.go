...
type Runner struct {
	pool *pgxpool.Pool
	dir  string
	// srcs нь migration файлын эх сурвалжууд — нэгтгэгдээд бүхэлдээ
	// дугаараар эрэмбэлэгдэнэ. Хоосон бол dir-ээс уншина. Ингэснээр апп нь
	// суурийн embed хийсэн migration дээр өөрийн хавтсыг нэмж чадна.
	srcs []fs.FS
...
// NewFS нь олон эх сурвалжаас (жишээ нь суурийн embed.FS + аппын хавтас)
// migration уншдаг Runner-г бүтээнэ. Эх сурвалжууд нэгтгэгдээд файлын
// эхний дугаараар нийтэд нь эрэмбэлэгдэнэ — өөрөөр хэлбэл суурийн 1..999
// нь аппын 1000+ -аас өмнө ажиллана.
func NewFS(pool *pgxpool.Pool, sources ...fs.FS) *Runner {
	return &Runner{pool: pool, srcs: sources}
}

// fileRef нь аль эх сурвалжийн аль файл болохыг заана.
type fileRef struct {
	fsys fs.FS
	name string
}
...
func (r *Runner) listFiles(action string) ([]fileRef, error) {
	srcs := r.srcs
	if len(srcs) == 0 {
		srcs = []fs.FS{os.DirFS(r.dir)}
	}

	var out []fileRef
	for _, fsys := range srcs {
		names, err := fs.Glob(fsys, fmt.Sprintf("*.%s.sql", action))
		if err != nil {
			return nil, errors.New("glob migration files")
		}
		for _, n := range names {
			out = append(out, fileRef{fsys: fsys, name: n})
		}
	}

	// Лексикограф эрэмбэ ашиглаж БОЛОХГҮЙ: "10_" нь "1_"-ээс өмнө ордог
	// ('0' < '_') тул шинэ хоосон DB дээр 10-р migration 1-ээс түрүүлж
	// ажиллана. Файлын нэрний эхний дугаараар тоон эрэмбэлнэ. Эх сурвалж
	// хоорондоо ялгагдахгүй — бүгд нэг дараалалд орно.
	slices.SortFunc(out, func(a, b fileRef) int {
		if c := cmp.Compare(migrationNumber(a.name), migrationNumber(b.name)); c != 0 {
			return c
		}
...
func applyFile(ctx context.Context, conn *pgxpool.Conn, file fileRef, upName string, isUp bool) error {
	data, err := fs.ReadFile(file.fsys, file.name)
	if err != nil {
	...
	if _, err := tx.Exec(ctx, string(data)); err != nil {
		return fmt.Errorf("exec %s: %w", filepath.Base(file.name), err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit %s: %w", filepath.Base(file.name), err)
	}
	return nil
}
